document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateEtapa7() {
        if (!equipA.checked && !equipB.checked) {
            errorEtapa7.textContent = 'Por favor, escolha um kit de equipamento inicial.';
            errorEtapa7.classList.remove('hidden');
            return false;
        }
        errorEtapa7.classList.add('hidden');
        return true;
    }
    ...
    form.addEventListener('submit', function(e) {
        ...
        const etapa1Valida = validateEtapa1();
        const etapa7Valida = validateEtapa7();

        if (etapa1Valida && etapa7Valida) {
