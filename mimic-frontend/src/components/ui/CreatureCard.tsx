interface CreatureCardProps {
  id: number; // Necessário para as ações
  title: string;
  race: string; // Mapear para 'tipo' (ex: Dragão)
  size: string; // Mapear para 'tamanho' (ex: Enorme)
  image: string | null; 
  onEdit: (id: number) => void;   // Função de editar
  onDelete: (id: number) => void; // Função de deletar
}

const CreatureCard = ({ id, title, race, size, image, onEdit, onDelete }: CreatureCardProps) => {
  return (
    <div className="relative block bg-[#2D2D2D] rounded-lg shadow-2xl overflow-hidden group transition-all duration-300 hover:shadow-red-500/30 border border-transparent hover:border-red-500/50">
      ...
      {/* CORPO DO CARD */}
      <div className="p-5 flex space-x-6 items-start pb-16"> {/* pb-16 para dar espaço aos botões */}
        ...
        <div className="bg-[#444444] rounded-lg flex-shrink-0 ring-2 ring-gray-600 group-hover:ring-red-500 transition-all self-stretch w-32 h-32 overflow-hidden flex items-center justify-center text-gray-500">
          {image ? (
            ...
            // Ícone de Monstro/Caveira Genérico
