...
interface CharacterCardProps {
  id: number; // Precisamos do ID para passar para as funções
  title: string;
  race: string;
  charClass: string;
...
const CharacterCard = ({ id, title, race, charClass, image, onEdit, onDelete }: CharacterCardProps) => {
  return (
    <div className="relative block bg-[#2D2D2D] rounded-lg shadow-2xl overflow-hidden group transition-all duration-300 hover:shadow-red-500/30 border border-transparent hover:border-red-500/50">
      ...
      {/* CORPO DO CARD */}
      <div className="p-5 flex space-x-6 items-start pb-16"> {/* pb-16 dá espaço para os botões */}
      ...
      {/* BOTÕES DE AÇÃO (Position Absolute no Canto Inferior Direito) */}
